...
class MessagesCog(commands.Cog):
    ...
    async def handle_new_message(self, message):
        ...
        if (
            not ctx.valid
        ):  # Note: If the context is valid, then the message is a command and is being handled elsewhere.
            ...
            if message.author != self.bot.user:
                ...
                if action["intent"] == "quiet":
                    ...
                    if BotConfig.is_silent():
                        logger.info(
                            "Will not respond. The bot has currently been silenced."
                        )
                        return
                ...
                if r > 0.96:
                    if BotConfig.is_silent():
                        logger.info(
                            "Will not make sarcastic comment. The bot has currently been silenced."
                        )
                    else:
